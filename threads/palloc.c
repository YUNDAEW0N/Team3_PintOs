...
PAL_USER가 설정된 경우 페이지는 사용자 풀에서 얻어지며, 그렇지 않으면 커널 풀에서 얻어집니다.
FLAGS에 PAL_ZERO가 설정된 경우 페이지는 0으로 채워집니다.
사용 가능한 페이지가 없는 경우, PAL_ASSERT가 FLAGS에 설정된 경우를 제외하고 널 포인터를 반환합니다.
이때 커널은 패닉 상태에 빠집니다. */
void *
