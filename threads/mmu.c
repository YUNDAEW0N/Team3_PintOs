...
CREATE가 true인 경우 새로운 페이지 테이블이 생성되고 그것에 대한 포인터가 반환됩니다.
...
/* 커널 가상 주소에 대한 매핑을 가지고 있지만 사용자 가상 주소에 대한 매핑을 가지지 않은
새로운 페이지 맵 레벨 4(pml4)를 생성합니다.
새로운 페이지 디렉터리를 반환하거나 메모리 할당 실패 시 널 포인터를 반환합니다. */
uint64_t *
pml4_create (void) {
...
/* 페이지 디렉터리 PD를 CPU의 페이지 디렉터리 기본 레지스터에 로드합니다. */
void
...
/* 페이지 디렉터리 PD에서 사용자 가상 페이지 UPAGE를 "프레젠트 상태 아님"으로 표시합니다.
이후 페이지에 대한 접근은 페이지 부재가 발생합니다.
...
/* PML4의 가상 페이지 VPAGE에 대한 PTE가 더티인 경우,
즉, PTE가 설치된 이후에 페이지가 수정된 경우 true를 반환합니다.
VPAGE에 대한 PML4에 PTE가 없는 경우 false를 반환합니다. */
bool
...
/* PML4의 가상 페이지 VPAGE에 대한 PTE가 최근에 접근되었는지 여부를 반환합니다.
즉, PTE가 설치된 시간과 마지막으로 지워진 시간 사이에 접근되었는지 여부를 반환합니다.
...
/* 페이지 디렉터리 PD의 가상 페이지 VPAGE에 대한 PTE의 액세스된 비트를
ACCESSED로 설정합니다. */
void
